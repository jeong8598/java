...
public class MemberDTO_02 {
	...
	//Object클래스의  equals(Object obj)는 객체 자신과 주어진 객체(obj)를 비교한다. 같으면 true, 다르면 false.
	//여기에서는 회원명과 주민번호가 동일하면 동일 객체라고
	//그렇지 않으면 동일객체x
	@Override
	public boolean equals(Object obj) {
		//매개변수obj의 타입은 Object이므로 Object를 상속받은 클래스는 다들어올 수 있다.
		//따라서
		//매개변수 obj에 들어온 value가 MemberDTO_02클래스의 객체인지 확인
		//=> MemberDTO_02의 인스턴스이니?
		//A instanceof B : A가 B의 instance이면 true 그렇지 않으면 false
		/*1. Object타입의 매개변수 obj에는 모든 객체가 매개값으로 제공될 수 있으므로
		 * 		instanceof 연산자를 기준 객체와 동일타입인지 먼저 확인을 해야한다.
		 * 		동일타입이면 true를 반환*/
	 ...
	 * 		instanceof 연산자를 기준 객체와 동일타입인지 먼저 확인을 해야한다.
	 * 		동일타입이면 true를 반환
	 * 2. 비교 객체가 동일한 타입이라면 기준 객체타입으로 강제타입변환해서
	 * 		특정 필드값이 동일한 지 비교
	 */
}
